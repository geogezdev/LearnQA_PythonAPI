...
user_agents_list = [
    ...
    'Version/4.0 Mobile Safari/534.30', {'platform': 'Mobile', 'browser': 'No', 'device': 'Android'}),
    ...
    ' Edg/91.0.100.0', {'platform': 'Web', 'browser': 'Chrome', 'device': 'No'}),
    ('Mozilla/5.0 (iPad; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 '
    'Mobile/15E148 Safari/604.1', {'platform': 'Mobile', 'browser': 'No', 'device': 'iPhone'})
]


@pytest.mark.parametrize('user_agent, expected_values', user_agents_list)
class TestUserAgent:
    def test_user_agent(self, user_agent, expected_values):
        ...
        assert response_dict['platform'] == expected_values['platform']
        assert response_dict['browser'] == expected_values['browser']
        assert response_dict['device'] == expected_values['device']
